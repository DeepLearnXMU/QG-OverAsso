...
from modeling_t5 import T5ForConditionalGenerationWeighting
...
def main():
    ...
    model = T5ForConditionalGenerationWeighting.from_pretrained(
    ...
    elif data_args.data_lang == 'zh':
        stopwords = set()
        with open(data_args.stopwords_file, 'r') as f:
            for k in f.readlines():
                stopwords.add(k.strip())
        def word_tokenize(text):
            return tokenizer.tokenize(text)
    else:
        raise Exception('Must define language en or zh first!')
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] is not None and examples[summary_column][i] is not None:
                dialog = examples[text_column][i]
                if data_args.data_lang == 'en':
                    dialog_tokens, dialog_lemmas = word_tokenize(dialog)
                    ...
                    dialog_tokens = word_tokenize(dialog)
                    for query in examples[summary_column][i]:
                        if query != '不检索':
