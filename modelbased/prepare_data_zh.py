...
new_data = []
for k in range(3):
    data_file = f'../saved_data/data_dusinc/support_{k}th_fold.json'
    model_file = f'../saved_data/mengzi-t5-base-dusinc_{k}fold_exponent-0.5'
    tokenizer = AutoTokenizer.from_pretrained(model_file)
    model = AutoModelForSeq2SeqLM.from_pretrained(model_file).cuda()

    def word_tokenize(text):
        return [char for char in text if char not in '。，？的']
    ...
    for example in tqdm(raw_data):
        ...
        with torch.no_grad():
            ...
            new_example = {'dialogue': dialog, 'query': gold_refs, 'candidate': sorted_predictions,
                           'score': scores, 'beam_score': sorted_scores}
            new_data.append(new_example)

with jsonlines.open(f'../saved_data/data_dusinc/train_exponent-0.5_beam_{beam}.json', 'w') as writer:
