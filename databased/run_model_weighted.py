...
class DataTrainingArguments:
    ...
    exponent: Optional[float] = field(default=1)
...
def main():
    ...
    if data_args.lang == 'zh':
        tokenizer.add_tokens(['[话题]', '[位置]', '[用户]', '[机器]', '[对话]'])
    elif data_args.lang == 'en':
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] is not None and examples[summary_column][i] is not None:
                if isinstance(examples[summary_column][i], list):
                    ind = random.randint(0, len(examples[summary_column][i]) - 1)
                    query, degree = examples[summary_column][i][ind], examples['degree'][i][ind]
                    inputs.append(examples[text_column][i])
                    targets.append(query)
                    weights.append(degree ** data_args.exponent)
                else:
                    query, degree = examples[summary_column][i], examples['degree'][i]
                    inputs.append(examples[text_column][i])
                    targets.append(query)
                    weights.append(degree ** data_args.exponent)
