...
def main(args):
    ...
    def collate_fn(batch):
        ...
        if isinstance(args, Arguments_zh) and args.tokenized:
            labels = [torch.LongTensor(tokenizer.convert_tokens_to_ids(x) + [tokenizer.eos_token_id])[:args.max_target_length] for x in candidates]
            model_inputs["labels"] = pad_sequence(labels, batch_first=True, padding_value=-100)
        else:
    ...
    if args.do_train:
        train_dataset = RAQGDataset(args.train_file, args.topk, args.none_adjust)
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'test':
        ...
        args.do_eval = False
        if len(sys.argv) > 3:
            args.model_name_or_path = args.output_dir+f'/checkpoint-{sys.argv[3]}'
            args.output_dir = args.output_dir+f'/checkpoint-{sys.argv[3]}'
            args.predictions = args.output_dir + f'/generated_predictions.txt'
        args.batch_size = 16
    elif sys.argv[1] == 'train':
        print('do train')
        # if len(sys.argv) > 2 and sys.argv[2] == 'res':
        #     args.train_file = 'train_resg_ra.json'
        #     args.output_dir = '../saved_data/RAQG-resg'
        #     args.predictions = '../saved_data/RAQG-resg/generated_predictions.txt'
        # elif len(sys.argv) > 2 and sys.argv[2] == 'norm':
        #     args.norm = True
        #     args.topk = 8
        #     args.output_dir = '../saved_data/RAQG-norm'
        #     args.predictions = '../saved_data/RAQG-norm/generated_predictions.txt'
    main(args)
